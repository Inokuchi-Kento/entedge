...
type Client struct {
	...
	// Bug is the client for interacting with the Bug builders.
	Bug *BugClient
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Bug, c.Group, c.Image, c.Pet, c.Tag, c.User,
	} {
		n.Use(hooks...)
	}
}
...
// BugClient is a client for the Bug schema.
type BugClient struct {
	config
}

// NewBugClient returns a client for the Bug from the given config.
func NewBugClient(c config) *BugClient {
	return &BugClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `bug.Hooks(f(g(h())))`.
func (c *BugClient) Use(hooks ...Hook) {
	c.hooks.Bug = append(c.hooks.Bug, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `bug.Intercept(f(g(h())))`.
func (c *BugClient) Intercept(interceptors ...Interceptor) {
	c.inters.Bug = append(c.inters.Bug, interceptors...)
}

// Create returns a builder for creating a Bug entity.
func (c *BugClient) Create() *BugCreate {
	mutation := newBugMutation(c.config, OpCreate)
	return &BugCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Bug entities.
func (c *BugClient) CreateBulk(builders ...*BugCreate) *BugCreateBulk {
	return &BugCreateBulk{config: c.config, builders: builders}
}

// Update returns an update builder for Bug.
func (c *BugClient) Update() *BugUpdate {
	mutation := newBugMutation(c.config, OpUpdate)
	return &BugUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *BugClient) UpdateOne(b *Bug) *BugUpdateOne {
	mutation := newBugMutation(c.config, OpUpdateOne, withBug(b))
	return &BugUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *BugClient) UpdateOneID(id int) *BugUpdateOne {
	mutation := newBugMutation(c.config, OpUpdateOne, withBugID(id))
	return &BugUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Bug.
func (c *BugClient) Delete() *BugDelete {
	mutation := newBugMutation(c.config, OpDelete)
	return &BugDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *BugClient) DeleteOne(b *Bug) *BugDeleteOne {
	return c.DeleteOneID(b.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *BugClient) DeleteOneID(id int) *BugDeleteOne {
	builder := c.Delete().Where(bug.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &BugDeleteOne{builder}
}

// Query returns a query builder for Bug.
func (c *BugClient) Query() *BugQuery {
	return &BugQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeBug},
		inters: c.Interceptors(),
	}
}

// Get returns a Bug entity by its id.
func (c *BugClient) Get(ctx context.Context, id int) (*Bug, error) {
	return c.Query().Where(bug.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *BugClient) GetX(ctx context.Context, id int) *Bug {
...
// QueryTags queries the tags edge of a Bug.
func (c *BugClient) QueryTags(b *Bug) *TagQuery {
	query := (&TagClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := b.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(bug.Table, bug.FieldID, id),
...
func (c *BugClient) Hooks() []Hook {
	return c.hooks.Bug
}

// Interceptors returns the client interceptors.
func (c *BugClient) Interceptors() []Interceptor {
	return c.inters.Bug
}

func (c *BugClient) mutate(ctx context.Context, m *BugMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&BugCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&BugUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&BugUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&BugDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Bug mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Bug, Group, Image, Pet, Tag, User []ent.Hook
	}
	inters struct {
		Bug, Group, Image, Pet, Tag, User []ent.Interceptor
	}
)
