...
func (ic *ImageCreate) SetData(b []byte) *ImageCreate {
	ic.mutation.SetData(b)
	return ic
}
...
func (ic *ImageCreate) check() error {
	if _, ok := ic.mutation.Data(); !ok {
		return &ValidationError{Name: "data", err: errors.New(`ent: missing required field "Image.data"`)}
	}
	return nil
}
...
func (ic *ImageCreate) createSpec() (*Image, *sqlgraph.CreateSpec) {
	...
	if value, ok := ic.mutation.Data(); ok {
		_spec.SetField(image.FieldData, field.TypeBytes, value)
		_node.Data = value
	}
	return _node, _spec
}
