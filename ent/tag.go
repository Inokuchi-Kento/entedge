...
func (*Tag) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case tag.FieldID, tag.FieldBugID:
			values[i] = new(sql.NullInt64)
		case tag.FieldTag:
			values[i] = new(sql.NullString)
		case tag.ForeignKeys[0]: // bug_id
			values[i] = new(sql.NullInt64)
		default:
...
func (t *Tag) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case tag.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field bug_id", value)
			} else if value.Valid {
				t.bug_id = new(int)
				*t.bug_id = int(value.Int64)
			}
		default:
...
func (t *Tag) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", t.BugID))
	builder.WriteString(", ")
	builder.WriteString("tag=")
